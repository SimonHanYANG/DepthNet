...
'''
    Func: 计算给出 tensor 的通道均值和方差
    Input: tensor (B, C, H, W)
    p.s.: eps: 为了防止除 0 错，给方差加上了常熟 eps，默认 1e-5
...
'''
    Func: CrossNorm methods 将原风格 feature map (content_feat) 迁移到目标 feateure map (style_feat) 上
    Input:
        content_feat: 原风格 feature map
        style_feat: 目标风格 feature map
    Return: CrossNorm 结果
'''
...
'''
    Func: 随即对给定 tensor 剪裁出一小块范围，若剪裁的大小未达到指定的比例 bbx_thres, 则重新裁剪，直到达到要求
    Input:
        bbx_thres: 指定裁剪的比例
    Return: 返回裁剪区域的两个坐标 (bbx1, bby1), (bbx2, bby2)
'''
